"""MS-AR canonical track: MarkovAutoregression(k=2, order=1, sw_var=True, trend='c')
...
def main() -> None:
    ...
    print(f"NaN in log_rv: {int(log_rv.isna().sum())}")
    print()

    print("Running rolling_forecast_msar_rv "
          "(k_regimes=2, order=1, switching_variance=True, trend='c')")
    ...
    df = pd.DataFrame(
        {
            ...
            "y_msar_pred": out.loc[valid_dates, "y_msar_pred_ffill"].to_numpy(dtype=np.float64),
        }
    )
    ...
    _vol_pct_metrics("MS-AR (ffill)", df, "y_msar_pred")
        ...
        df_clean = pd.DataFrame(
            {
                ...
                "y_msar_pred": out.loc[clean_dates, "y_msar_pred"].to_numpy(dtype=np.float64),
            }
        )
        _vol_pct_metrics("MS-AR (clean)", df_clean, "y_msar_pred")
    else:
        print("  (no fit failures — clean and ffill columns are identical)")

    out_path = OUT_DIR / "msar_canonical_predictions.csv"
